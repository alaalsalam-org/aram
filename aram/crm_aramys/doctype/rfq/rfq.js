...
frappe.ui.form.on("RFQ", {
	...
	make_supplier_quotation: function(frm) {
		frappe.model.open_mapped_doc({
			method: "aram.aram.crm_aramys.doctype.rfq.rfq.make_supplier_quotation",
			frm: frm
		})
	},
	...
	make_request_for_quotation: function(frm) {
		frappe.model.open_mapped_doc({
			method: "aram.aram.crm_aramys.doctype.rfq.rfq.make_request_for_quotation",
			frm: frm
		})
	},
...
aram.rfq.Opportunity = class Opportunity extends frappe.ui.form.Controller {
	onload() {
	...
	create_quotation() {
		frappe.model.open_mapped_doc({
			method: "aram.aram.crm_aramys.doctype.rfq.rfq.make_quotation",
			frm: cur_frm
		})
	}
...
extend_cscript(cur_frm.cscript, new aram.rfq.Opportunity({frm: cur_frm}));
...
cur_frm.cscript.item_code = function(doc, cdt, cdn) {
	...
	if (d.item_code) {
		return frappe.call({
			method: "aram.aram.crm_aramys.doctype.rfq.rfq.get_item_details",
			args: {"item_code":d.item_code},
			callback: function(r, rt) {
				console.log(rt, t);
				if(r.message) {
